...
export default function ArticlePage({ articles }) {
  ...
  useEffect(() => {
    async function gettingId() {
      const url = "http://www.sesejose.com/kea/wp_boliglaan/wp-json/wp/v2/article/" + id + "?_embed";
      console.log(id);
...
export async function getServerSideProps() {
  const url = "http://www.sesejose.com/kea/wp_boliglaan/wp-json/wp/v2/article?_embed=1&per_page=4";
  // Await then execute the code.
  const res = await fetch(url); // Fetchs the data (await)
